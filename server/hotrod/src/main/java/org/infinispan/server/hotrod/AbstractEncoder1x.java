...
abstract class AbstractEncoder1x implements VersionedEncoder {
   ...
   public void writeResponse(Response r, ByteBuf buf, EmbeddedCacheManager cacheManager, HotRodServer server) {
      if (r instanceof ResponseWithPrevious) {
         ...
         if (bgr.status == OperationStatus.Success) {
            ...
            if (bgr.count != 0) {
               if (trace)
                  log.tracef("About to write (max) %d messages to the client", Integer.toString(bgr.count));
               count = bgr.count;
            } else {
   ...
   protected void writeCommonHashTopologyHeader(ByteBuf buffer, int viewId,
      ...
      if (trace)
         log.tracef("Topology will contain %d addresses", Integer.toString(numServers));
   }
