...
public class ClusteredLockSplitBrainTest extends BasePartitionHandlingTest {
   ...
   public void testLockUseAfterPartitionWithoutMajority() throws Throwable {
      ...
      assertNonLockAcquisition(lock0);
      assertNonLockAcquisition(lock1);
      assertNonLockAcquisition(lock2);
      assertNonLockAcquisition(lock3);
   }

   private void assertNonLockAcquisition(ClusteredLock lock) {
      assertTrue(await(lock.tryLock(5, TimeUnit.SECONDS).exceptionally(ex -> {
         assertException(ClusteredLockException.class, ex);
         assertException(CompletionException.class, ex.getCause());
   ...
   private void assertFailureFromMinorityPartition(ClusteredLock lock3) {
      await(lock3.lock()
            .thenRun(() -> {
               ...
               assertException(CompletionException.class, ex.getCause().getCause());
               assertException(AvailabilityException.class, ex.getCause().getCause().getCause());
               return null;
            }));
   }
}
