...
public class SemaphoreCompletionService<T> implements CompletionService<T> {
   ...
   public Future<T> submit(final Callable<T> task) {
      ...
      if (trace) log.tracef("New task submitted, tasks in queue %d, available permits %d", (Integer)queue.size(), (Integer)semaphore.availablePermits());
      executeFront();
      return futureTask;
   }
   ...
   private class QueueingTask extends FutureTask<T> {
      ...
      private void runInternal() {
         try {
            if (trace) log.tracef("Task started, tasks in queue %d, available permits %d", (Integer)queue.size(), (Integer)semaphore.availablePermits());
            super.run();
         } finally {
            completionQueue.offer(this);
            if (trace) log.tracef("Task finished, tasks in queue %d, available permits %d", (Integer)queue.size(), (Integer)semaphore.availablePermits());
         }
      }
   }
