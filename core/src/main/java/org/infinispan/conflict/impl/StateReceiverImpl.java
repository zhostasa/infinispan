...
public class StateReceiverImpl<K, V> implements StateReceiver<K, V> {
   ...
   class SegmentRequest {
      ...
      synchronized CompletableFuture<List<Map<Address, CacheEntry<K, V>>>> requestState() {
         assert future == null;
         if (trace) log.tracef("Attempting to receive replicas for segment %s from %s with topology %s", (Integer)segmentId, replicaHosts, topology);
      ...
      synchronized void cancel(Throwable throwable) {
         log.debugf(throwable, "Cancelling request for segment %s on cache %s", (Integer)segmentId, cacheName);
